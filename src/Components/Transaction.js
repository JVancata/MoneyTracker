    ...
    render = () => {
        ...
        return (
            <div className={inputs.add ? "w3-row w3-pale-green w3-padding" : "w3-row w3-pale-red w3-padding"} key={index}>
                <div className="w3-col s7" onClick={() => this.toggleEdit(index)}>{!editing ? <p>{inputs.description}</p> : <input onKeyPress={this._handleKeyPress} onChange={this.onChange} type="text" name="description" value={inputs.description} />} </div>
                <div className="w3-col s3" onClick={() => this.toggleEdit(index)}>{!editing ? <p>{inputs.amount}</p> : <input onKeyPress={this._handleKeyPress} onChange={this.onChange} type="number" name="amount" value={inputs.amount} />}</div>
